...
class MY_Model extends CI_Model {
    ...
    try {
      ...
      if (is_array($dados) && !empty($dados)):
        if (!is_null($criterio) && !empty($criterio)):
          ...
          $this->db->set($class::CRIADO, 'SYSDATE', FALSE);
          if (isset($this->data['user'])) :
            $this->db->set($class::USUARIO, $this->data['user'][model_usuario::ID]);
          endif;
          $this->db->insert($class::TABELA, $dados);
          $resposta = TRUE;
        endif;
      else:
        throw new Exception('show_stack_bar_top("error", "Erro", "Erro de cadastro!")');
      endif;
      return $resposta;
    } catch (Exception $exc) {
  ...
  private function atualizar($dados = array(), $criterio = NULL) {
    ...
    try {
      ...
      if (!$erro) {
        $this->db->set($class::MODIFICADO, 'SYSDATE', FALSE);
        $this->db->update($class::TABELA, $dados);
        return TRUE;
      } else {
        throw new Exception('show_stack_bar_top("error", "Erro", "Falha ao atualizar dado.")');
      }
    } catch (Exception $exc) {
      return $exc->getMessage();
    }
