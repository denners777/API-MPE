<form id="formulario" class="form-horizontal" role="form" method="post" action="<?php echo base_url('nucleo/acesso/atualizar'); ?>" onsubmit="overlay(true)">
  <fieldset class="col-sm-6 col-sm-offset-3 well">
    <div class="header">Novo Modulo</div>
    <input type="hidden" name="<?php echo model_acesso::ID; ?>" id="id" value="<?php echo $acesso[model_acesso::ID]; ?>" />

    <div class="form-group">
      <label for="usuarios" class="col-sm-3 control-label">Usuario</label>
      <div class="col-sm-9">
        <input type="hidden" id="usuarios" name="<?php echo model_acesso::USUARIOACESSO; ?>" value="<?php echo $acesso[model_acesso::USUARIOACESSO]; ?>"/>
        <input type="text" id="usuarios_view" class="form-control" placeholder="Digite o nome ou chapa do funcionário" value="<?php echo $acesso[model_funcionario::NOME]; ?>" />
        ...
        <select id="modulos" name="<?php echo model_acesso::MODULO; ?>" class="form-control" required>
          ...
          <?php foreach ($modulos as $value) :
            ...
            <option value="<?php echo $value[model_modulo::ID]; ?>"
                    <?php echo $acesso[model_acesso::MODULO] == $value[model_modulo::ID] ? 'selected' : '' ?>>
        ...
        <select id="privilegios" name="<?php echo model_acesso::PRIVILEGIO; ?>" class="form-control" required>
          ...
          <?php foreach ($privilegios as $value) :
            ...
            <option value="<?php echo $value[model_privilegio::ID]; ?>"
                    <?php echo $acesso[model_acesso::PRIVILEGIO] == $value[model_privilegio::ID] ? 'selected' : '' ?>>
